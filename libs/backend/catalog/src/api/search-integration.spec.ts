...
const mockRedis = { get: mock(() => Promise.resolve(null)), set: mock(() => Promise.resolve('OK')) } as any;
...
// Handler
const searchHandler = new SearchMediaHandler(mockRepo, mockRedis, mockIgdb, mockTmdb, mockGbooks);
...
describe('Search Integration (API -> Handler)', () => {
    ...
    it('should return 200 and grouped structure (Local)', async () => {
        ...
        const res = await app.handle(new Request('http://localhost/media/search?q=test'));
    ...
    it('should trigger remote search and return structured result', async () => {
        ...
        const res = await app.handle(new Request('http://localhost/media/search?q=remote'));
        expect(res.status).toBe(200);
    ...
    it('should validate query length', async () => {
        const res = await app.handle(new Request('http://localhost/media/search?q=a'));
    ...
    it('should handle validation error for empty query', async () => {
        const res = await app.handle(new Request('http://localhost/media/search?q='));
