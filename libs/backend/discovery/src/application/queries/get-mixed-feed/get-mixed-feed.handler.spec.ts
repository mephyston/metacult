...
describe('GetMixedFeedHandler', () => {
  it('should return mixed feed from cache if available', async () => {
    const mockRedis = {
      get: mock(() =>
        Promise.resolve(JSON.stringify([{ type: 'MEDIA', data: {} }])),
      ),
    ...
    const handler = new GetMixedFeedHandler(
      mockRedis,
      mockMediaSearcher,
      mockAds,
    );
  ...
  it('should fetch dependencies and mix feed on cache miss', async () => {
    ...
    // Total: 10 + 2 = 12 items.
    expect(result.getValue()).toHaveLength(12);
    expect(result.getValue()[5]?.type).toBe('SPONSORED');
