...
async function main() {
  logger.info('🌱 Envoi des Jobs de Seed à la Queue...');
  ...
  for (const seed of seeds) {
    logger.info({ type: seed.type, id: seed.id }, '🚀 Ajout en queue');
  ...
  logger.info(
    "✅ Jobs de Seed en file d'attente ! Assurez-vous que le Worker tourne.",
  );
  process.exit(0);
}

main().catch((err) => {
  logger.error({ err }, '❌ Echec du Seeding');
  process.exit(1);
});
