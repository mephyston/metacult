...
mock.module('../../../../../identity/src/api/middleware/auth.middleware.ts', () => {
    return {
        isAuthenticated: (app: any) => app.derive(() => ({
...
// Spies
const mockGetRandomPairForUser = mock(() => Promise.resolve([{ id: '1', title: 'A' }, { id: '2', title: 'B' }]));
const mockAddDuelResult = mock(() => Promise.resolve());

spyOn(DrizzleDuelRepository.prototype, 'getRandomPairForUser').mockImplementation(mockGetRandomPairForUser as any);
spyOn(RankingQueue.prototype, 'addDuelResult').mockImplementation(mockAddDuelResult as any);
...
describe('Duel Controller', () => {
    ...
    it('POST /duel/vote should record vote', async () => {
        ...
        const response = await app.handle(new Request('http://localhost/duel/vote', {
            method: 'POST',
