...
<template>
    <div class="review-deck relative w-full h-[600px] max-w-md mx-auto">
        <div v-if="currentIndex >= items.length"
            ...
            <h3 class="text-2xl font-bold text-card-foreground mb-2">You're all caught up!</h3>
            <p class="text-muted-foreground">Check back later for more content to review.</p>
        </div>
        ...
        <template v-else>
            ...
            -->
                <ReviewCard v-if="index === currentIndex || index === currentIndex + 1" :title="item.title"
                    :image="item.image" :category="item.category" :description="item.description"
                    :active="index === currentIndex"
                    :style="{ zIndex: items.length - index, transform: index > currentIndex ? 'scale(0.95) translateY(10px)' : '' }"
                    @swipe-left="handleVote('left')" @swipe-right="handleVote('right')" />
            </template>
        </template>
    </div>
</template>
