...
<template>
  <div
    class="review-card absolute top-0 left-0 w-full h-full transition-all duration-300 ease-out transform"
    :class="{
      'opacity-100 scale-100 z-10': active,
      'opacity-0 scale-95 z-0': !active,
    }"
  >
    <Card class="h-full flex flex-col shadow-2xl">
      ...
      <!-- Action Buttons -->
      <div
        class="px-6 py-6 bg-card border-t border-border flex justify-center gap-6"
      >
        <button
          class="w-16 h-16 rounded-full border-2 border-red-500 text-red-500 flex items-center justify-center hover:bg-red-500/10 transition-colors shadow-sm"
          @click="vote('left')"
        >
          <svg
        ...
        <button
          class="w-12 h-12 rounded-full border border-border text-muted-foreground flex items-center justify-center hover:bg-accent hover:text-accent-foreground transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
