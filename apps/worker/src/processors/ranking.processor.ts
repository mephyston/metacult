...
import { EloCalculator, RankingUpdateJob } from '@metacult/backend/ranking';
...
export const processRankingUpdate = async (job: Job<RankingUpdateJob>) => {
    ...
    console.log(`🏆 [RankingProcessor] Processing duel: ${winnerId} (Win) vs ${loserId} (Loss)`);

    // 1. Fetch Medias
    // Note: Drizzle executes queries in parallel if we don't await them sequentially
    const [winner, loser] = await Promise.all([
        db.select().from(mediaSchema.medias).where(eq(mediaSchema.medias.id, winnerId)).limit(1).then(rows => rows[0]),
        db.select().from(mediaSchema.medias).where(eq(mediaSchema.medias.id, loserId)).limit(1).then(rows => rows[0])
    ]);
    ...
    const { winnerNewElo, loserNewElo } = eloCalculator.calculateNewScores(winner.eloScore, loser.eloScore);

    // 3. Update DB
    // We update match_count and elo_score
    await db.transaction(async (tx) => {
        await tx.update(mediaSchema.medias)
            .set({
    ...
    console.log(`✅ [RankingProcessor] Update Complete: "${winner.title}" (${winner.eloScore} -> ${winnerNewElo}) vs "${loser.title}" (${loser.eloScore} -> ${loserNewElo})`);
};
