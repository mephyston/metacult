...
export const useGuestSync = () => {
        ...
        if (syncParam) {
            try {
                ...
                if (Array.isArray(swipes) && swipes.length > 0) {
                    console.log(`[GuestSync] Found ${swipes.length} pending swipes. Storing...`)
    ...
    const flushSync = async () => {
        ...
        try {
            ...
            console.log('[GuestSync] Pending Swipes Raw:', JSON.stringify(pendingSwipes.value));
            ...
            if (payload.length === 0) {
                console.warn('[GuestSync] No valid interactions to sync (all filtered out). Clearing storage.')
                pendingSwipes.value = []
                return
            }
