...
const handleInteraction = async (payload: any) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('[Discover] Interaction failed:', response.status, errorText)
        } else {
...
<template>
    <div class="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] p-4">
        ...
        <div v-if="isLoading && queue.length === 0" class="flex flex-col items-center gap-4 animate-pulse">
                <div class="w-80 h-[500px] bg-muted rounded-xl"></div>
                <div class="h-4 w-32 bg-muted rounded"></div>
            </div>

<div v-else-if="queue.length > 0" class="w-full">
            <SwipeDeck :items="queue" @interaction="handleInteraction" @empty="handleEmpty" />
            </div>

            <div v-else class="text-center space-y-4">
                <h2 class="text-2xl font-bold">Tout est calme...</h2>
                <p class="text-muted-foreground">Tu as vu tout ce qu'on avait pour l'instant ! Reviens plus tard.</p>
                <p class="text-xs text-muted-foreground">API URL: {{ apiUrl }}</p>
                <p class="text-xs text-muted-foreground">User: {{ user?.email || 'Not logged in' }}</p>
